...
def worker(passw, hash_read):
    ...
    try:
        hashed_password = hashlib.sha384(passw.strip().encode('ISO-8859-1', errors='ignore')).hexdigest()
...
def crack_sha384(format_h,path_wordlist1,target_hash_file):
	
	length_wordlist = lenWordlist.calc_len_wordlist(path_wordlist1)
	
	if format_h.lower() == 'sha384':
...
if __name__ == "__main__":
	...
	#python3 Cr_sha384.py sha384 path_wordlist target_hashfile
	if len(sys.argv) != 4:
		print("usage: python3 Cr_sha384.py sha384 path_wordlist target_hashfile")
		sys.exit(1)
	...
	crack_sha384(format_h,path_wordlst,target_hash_f)
