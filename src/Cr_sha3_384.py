...
def worker(passw, hash_read):
    ...
    try:
        hashed_password = hashlib.sha3_384(passw.strip().encode('ISO-8859-1', errors='ignore')).hexdigest()
...
def crack_sha3_384(format_h,path_wordlist1,target_hash_file):
	
	length_wordlist = lenWordlist.calc_len_wordlist(path_wordlist1)
	
	if format_h.lower() == 'sha3_384':
		...
		if length_wordlist > 100000:
			...
			pool = Pool(36) #36işlem kullanarak çoklu işlem havuzu başlatılıyor.
...
if __name__ == "__main__":
	...
	#python3 Cr_sha3_384.py sha3_384 path_wordlist target_hashfile
	if len(sys.argv) != 4:
		print("usage: python3 Cr_sha3_384.py sha3_384 path_wordlist target_hashfile")
		sys.exit(1)
	...
	crack_sha3_384(format_h,path_wordlst,target_hash_f)
