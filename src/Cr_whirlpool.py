...
def worker(passw, hash_read):
    ...
    try:
        hashed_password = hashlib.new('whirlpool', passw.strip().encode('ISO-8859-1', errors='ignore')).hexdigest()
...
def crack_whirlpool(format_h,path_wordlist1,target_hash_file):
	
	length_wordlist = lenWordlist.calc_len_wordlist(path_wordlist1)
	
	if format_h.lower() == 'whirlpool':
		
		#passw = None
		if length_wordlist > 100000:
			#process
			#multi processing attack(32 per)
...
if __name__ == "__main__":
	...
	#python3 Cr_whirlpool.py whirlpool path_wordlist target_hashfile
	if len(sys.argv) != 4:
		print("usage: python3 Cr_whirpool.py whirpool path_wordlist target_hashfile")
		sys.exit(1)
	...
	crack_whirlpool(format_h,path_wordlst,target_hash_f)
