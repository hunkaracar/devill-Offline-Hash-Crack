...
def worker(passw,hash_read):
	...
	try:
		...
		hashed_password = hashlib.blake2b(passw.strip().encode('ISO-8859-1', errors='ignore'), digest_size=64).hexdigest()
...
def crack_blake2b(format_h, path_wordlst1, target_hash_f):
	
    len_wordlist = lenWordlist.calc_len_wordlist(path_wordlst1)
    

    if format_h.lower() == 'blake2b':
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 4:
        print("usage: python3 Cr_blake2b.py blake2b path_wordlist target_hashfile")
        sys.exit(1)
    ...
    crack_blake2b(format_h, path_wordlst, target_hash_f)
